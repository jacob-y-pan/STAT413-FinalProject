...
## Simulated Data
For this first section, we will generate a polynomial dataset and use Mahalanobis distance to detect outliers.

```{r}
# Generate simulated polynomial data, coloring based on Mahalanobis distance
x1poly = runif(100, min = -1, max = 1)
ypoly = 5*x1poly^2 + 2*x1poly + 4 + rnorm(100, mean = 0, sd = 1)
...
```
Now, we are going to local outlier factor to detect outliers in the simulated data.
...
Let's plot and compare to the Mahalanobis distance results.

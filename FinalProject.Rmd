...
## Simulated Data
For this first section, we will generate a polynomial dataset.
...
poly_train = poly_df[1:80, ]
poly_test = poly_df[81:100, ]
```
We also want to add our explicit outliers - these will be points that are at least 3 standard deviations away from the mean.
...
Now let's calculate the Mahalanobis distance for each point in the training set, and plot the data with the Mahalanobis distance ellipses.
...
Now we are going to use Isolation Forest to detect outliers in the simulated data.
...
Now, we are going to use local outlier factor to detect outliers in the simulated data.
...
Finally, we'll use KNN to detect outliers in the simulated data.
...
for (k in 2:5) {
  for (eps in seq(0, 2, .1)) {
    # Create the model using the KNN results
    knn_result = dbscan(poly_train, eps = eps, minPts = k)
    knn_outliers = which(knn_result$cluster == 0)
    # Check if knn_outliers empty
    if (length(knn_outliers) == 0 || length(knn_outliers) > 0.1 * nrow(poly_train)) {
      next
    }
    model = lm(ypoly ~ x1poly+I(x1poly^2), data = poly_train[-knn_outliers, ])
    
    # Get the model's MSE on the test set
    test_pred = predict(model, poly_train)
    test_mse = mean((test_pred - poly_train$ypoly)^2)
...
knn_result = dbscan(poly_train, eps = best_eps, minPts = best_minpts) # Optimal parameters found in the loop
knn_outliers = which(knn_result$cluster == 0)
knn_outliers

knn_outliers_pts = poly_train[knn_outliers, ]
```
...
# Plot the data with the outliers
ggplot(poly_train, aes(x = x1poly, y = ypoly)) + 
  geom_point(aes(color=knn_result$cluster)) + 
  geom_polygon(data = poly_ellipse, alpha = 0.2, fill = "blue", color = "blue") +
...
Let's now generate a mutli-linear regression model using this train data after removing points based on Mahalanobis and then Local Outlier Factor, and then find the MSE with the test data.
```{r}
...
Let's also compare the R-Squared values of the two models.
...
ggplot(r2_df, aes(x = method, y = r2)) + 
  ...
  theme_minimal()
```
We can see that with MSE, Mahalanobis takes the crown, but with R-Sqaured Isolation Forest performs pretty well, with the same degree as Mahalanobis. Unfortunately in this case it seems as KNN and LOF don't perform well with MSE or R-Squared.
...
Now we will calculate the mahanalobis distance for each point to determine outliers
```{r}
...
```{r}
if_model = isolation.forest(complex_train, ntree = 100, ndim = 2)
...
# Find the optimal values for eps and MinPts
for (k in 2:5) {
  for (eps in seq(0, 5, 0.5)) {
    # Create the model using the KNN results
    knn_result = dbscan(complex_train, eps = eps, minPts = k)
    knn_outliers = which(knn_result$cluster == 0)
    # Check if knn_outliers empty
    if (length(knn_outliers) == 0 || length(knn_outliers) > 0.1 * nrow(complex_train)) {
      next
    }
    model = lm(ycomplex~x1complex+I(x1complex^2)+x2complex+I(x2complex^2), data = complex_train[-knn_outliers, ])
    
    # Get the model's MSE on the test set
    test_pred = predict(model, complex_train)
    test_mse = mean((test_pred - complex_train$ycomplex)^2)
...
# Best test MSE, eps, and MinPts values
c(best_test_mse, best_eps, best_minpts)
knn_result = dbscan(complex_train, eps = best_eps, minPts = best_minpts) # Optimal parameters found in the loop
knn_outliers = which(knn_result$cluster == 0)
knn_outliers
```
...
# Plot the data, x1 first
ggplot(complex_train, aes(x = x1complex, y = ycomplex)) + 
  geom_point(aes(col=knn_result$cluster)) +
...
# Remove from knn
complex_train_knn = complex_train[-knn_outliers, ]
